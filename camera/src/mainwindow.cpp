...
static QWidget *createPicSelectableLineEditOptionHandle(QObject *opt)
{
    ...
    if (picStrElideText[0] == '~' && picStrElideText != "~/Pictures/Camera") {
        picStrElideText.replace(0, 1, QDir::homePath());
        lastPicPath = picStrElideText;
    } else if (picStrElideText[0] == '~' && picStrElideText == "~/Pictures/Camera") {
...
static QWidget *createVdSelectableLineEditOptionHandle(QObject *opt)
{
    ...
    if (curVideoSettingPath.contains("~/")) {
    ...
    if (VideoStrElideText[0] == '~' && VideoStrElideText != "~/Videos/Camera") {
        VideoStrElideText.replace(0, 1, QDir::homePath());
        lastVideoPath = VideoStrElideText;
    } else if (VideoStrElideText[0] == '~' && VideoStrElideText == "~/Videos/Camera") {
...
void CMainWindow::initThumbnailsConn()
{
    ...
    //系统文件变化信号
    //已验证通过文管删除文件，不会有fileChanged信号产生，只有directoryChaneged信号产生
