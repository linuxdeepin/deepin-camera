...
int encoder_process_next_video_buffer(encoder_context_t *encoder_ctx)
{
    ...
    if (HW_VAAPI_OK == is_vaapi)
        encoder_encode_video_vaapi(encoder_ctx, video_ring_buffer[video_read_index].frame);
    else
...
int encoder_flush_video_buffer(encoder_context_t *encoder_ctx)
{
    ...
    // {
    if (HW_VAAPI_OK == is_vaapi)
        encoder_encode_video_vaapi(encoder_ctx, NULL);
    else
 ...
 * returns: none
 */
void encoder_video_close(encoder_video_context_t *enc_video_ctx)
{
    if (!enc_video_ctx)
        return;
    encoder_codec_data_t *video_codec_data = NULL;
...
static char *vm_err22str(int errnum)
{
    char *errbuf = (char[AV_ERROR_MAX_STRING_SIZE]) {0};
    getAvutil()->m_av_strerror(errnum, errbuf, 64);
    return errbuf;
}
...
static int set_hwframe_ctx(AVCodecContext *ctx, AVBufferRef *hw_device_ctx, int width, int height)
{
    ...
    if (!(hw_frames_ref = getAvutil()->m_av_hwframe_ctx_alloc(hw_device_ctx))) {
    ...
    if ((err = getAvutil()->m_av_hwframe_ctx_init(hw_frames_ref)) < 0) {
        fprintf(stderr, "Failed to initialize VAAPI frame context."
                "Error code: %s\n", vm_err22str(err));
        getAvutil()->m_av_buffer_unref(&hw_frames_ref);
        return err;
    }
    ctx->hw_frames_ctx = getAvutil()->m_av_buffer_ref(hw_frames_ref);
...
static encoder_video_context_t *encoder_video_init_vaapi(encoder_context_t *encoder_ctx)
{
   ...
   if(!check_vaapi_support())
   {
    ...
    encoder_video_context_t *enc_video_ctx = calloc(1, sizeof(encoder_video_context_t));
    if (enc_video_ctx == NULL) {
        fprintf(stderr, "ENCODER: FATAL memory allocation failure (e_ncoder_video_init): %s\n", strerror(errno));
        is_vaapi =  HW_VAAPI_FAIL2;
    ...
    if (video_codec_data == NULL)    {
        fprintf(stderr, "ENCODER: FATAL memory allocation failure (e_ncoder_video_init): %s\n", strerror(errno));
        is_vaapi =  HW_VAAPI_FAIL4;
         fprintf(stderr, "FAIL to encoder_video_init_vaapi:%d.\n", is_vaapi);
         return NULL;
    }


    int err = getAvutil()->m_av_hwdevice_ctx_create(&hw_device_ctx, AV_HWDEVICE_TYPE_VAAPI,  NULL, NULL, 0);
    if (err < 0) {
        fprintf(stderr, "Failed to create a VAAPI device. Error code: %s\n", vm_err22str(err));
        is_vaapi =  HW_VAAPI_FAIL5;

         fprintf(stderr, "FAIL to encoder_video_init_vaapi:%d.\n", is_vaapi);
         free(video_codec_data);
         return NULL;
    }
//  video_defaults->codec_name = "h264_vaapi";
    video_codec_data->codec = getLoadLibsInstance()->m_avcodec_find_encoder_by_name("h264_vaapi"); (video_defaults->codec_name);

    if (!video_codec_data->codec)    {
        /*we will use raw data so free the codec data*/
        free(video_codec_data);
        fprintf(stderr, "ENCODER: libav video codec (%i) not found - using raw input\n", video_defaults->codec_id);
        is_vaapi =  HW_VAAPI_FAIL6;
         fprintf(stderr, "FAIL to encoder_video_init_vaapi:%d.\n", is_vaapi);
         free(video_codec_data);
         return NULL;
    }
...
//  video_codec_data->codec_context->pix_fmt =  video_defaults->pix_fmt; //only yuv420p available for mpeg
    video_codec_data->codec_context->pix_fmt =  AV_PIX_FMT_VAAPI;
    if (video_defaults->fps)
        video_codec_data->codec_context->time_base = (AVRational) {1, video_defaults->fps}; //use codec properties fps
    else if (encoder_ctx->fps_den >= 5)
        video_codec_data->codec_context->time_base = (AVRational) {encoder_ctx->fps_num, encoder_ctx->fps_den}; //default fps (for gspca this is 1/1)
    ...
    if ((ret = set_hwframe_ctx(video_codec_data->codec_context, hw_device_ctx, video_codec_data->codec_context->width, video_codec_data->codec_context->height)) < 0) {
        fprintf(stderr, "Failed to set hwframe context.\n");
        is_vaapi =  HW_VAAPI_FAIL8;
        getAvutil()->m_av_buffer_unref(&hw_device_ctx);
        fprintf(stderr, "FAIL to encoder_video_init_vaapi:%d.\n", is_vaapi);
        free(video_codec_data);
        return NULL;
   ...
   if ((ret = getLoadLibsInstance()->m_avcodec_open2(video_codec_data->codec_context,video_codec_data->codec,&video_codec_data->private_options)) < 0){
    ...
    vaapi_coded_w = video_codec_data->codec_context->width;
    vaapi_coded_h = video_codec_data->codec_context->height;
    out_uuvv = (uint8_t *)malloc(vaapi_coded_w * vaapi_coded_h / 2);
    out_yy = (uint8_t *)malloc(vaapi_coded_w * vaapi_coded_h);
...
int encoder_encode_video_vaapi(encoder_context_t *encoder_ctx, void *input_frame)
{
    ...
    /*raw - direct input no software encoding*/
    if (encoder_ctx->video_codec_ind == 0) {
        fprintf(stderr, "Error code: encoder_ctx->video_codec_ind == 0 \n");
        return (outsize);
    }
    ...
    if (input_frame != NULL) {
        // // prepare_video_frame_nv12
        //     /*assertions*/
        assert(video_codec_data);
        assert(input_frame);
        int width = encoder_ctx->video_width;
        int height = encoder_ctx->video_height;
        int size = width * height;
        /*  by ffmpeg way
            // int linesize[4];
            // // swsframe   = getAvutil()->m_av_frame_alloc();

            // swsContext = getLoadLibsInstance()->m_sws_getContext(width, height, (enum AVPixelFormat)video_codec_data->frame->format, \
            //                                                         width, height, AV_PIX_FMT_NV12, SWS_BILINEAR, NULL, NULL, NULL);
            // getAvutil()->m_av_image_alloc(swsframe->data , linesize, width, height, AV_PIX_FMT_NV12, 16);  //AV_PIX_FMT_YUYV422   AV_PIX_FMT_NV12
            // getLoadLibsInstance()->m_sws_scale(swsContext, video_codec_data->frame->data, video_codec_data->frame->linesize, 0, height, swsframe->data, linesize);
        ...
        */
         ...
         * vaapi_coded_w/h hold the aligned dimensions the encoder was opened with. */
        uint8_t *y_plane = input_frame;
        uint8_t *uv_plane = out_uuvv;
    ...
    if (!(hw_frame = getAvutil()->m_av_frame_alloc()))
        err = AVERROR(ENOMEM);

    if ((getAvutil()->m_av_hwframe_get_buffer(video_codec_data->codec_context->hw_frames_ctx, hw_frame, 0)) < 0) {
        fprintf(stderr, "Error code: av_hwframe_get_buffer.\n");

    }
    if (!hw_frame->hw_frames_ctx) {
        err = AVERROR(ENOMEM);
        fprintf(stderr, "Error code: hw_frame->hw_frames_ctx.\n");
    }

    if ((err = getAvutil()->m_av_hwframe_transfer_data(hw_frame, video_codec_data->frame, 0)) < 0) {
        //   if (( err = getAvutil()->m_av_hwframe_transfer_data(hw_frame, swsframe, 0)) < 0) {
    ...
    ret = libav_send_encode(video_codec_data->codec_context, hw_frame);
    ...
    getAvutil()->m_av_frame_free(&hw_frame);
    return (outsize);
#endif
}
