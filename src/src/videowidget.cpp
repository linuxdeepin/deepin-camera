...
void videowidget::ReceiveMajorImage(QImage *image, int result)
{
    if (!image->isNull()) {
            ...
            }
        {
            ...
            //qInfo()<<"widgetwidth :" << this->width() << " widgetheight: " << this->height() << endl;
        ...
        if (VIDEO_STATE == NORMALVIDEO) {
            ...
            if (--m_curTakePicTime == 0) {
                ...
                QThread *thread = QThread::create([ = ]() {
                    ...
                    while (true) {
                        if (nCount > 50) {
                            qInfo() << "too long to emit takePicDone";
                            break;
                        }
                ...
                QThread *thread = QThread::create([ = ]() {
                    ...
                    while (true) {
                        if (nCount > 50) {
                            qInfo() << "too long to emit takePicOnce";
                            break;
                        }
                        if (!m_imgPrcThread->m_bTake) {
                            break;
