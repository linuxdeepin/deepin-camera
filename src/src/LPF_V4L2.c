...
static const struct rkisp_api_ctx * rkisp_ctx = NULL;

void close_rkisp_ctx()
{
    if(rkisp_ctx){
        rkisp_stop_capture(rkisp_ctx);
        rkisp_close_device(rkisp_ctx);
        rkisp_ctx = NULL;
    }
}
...
{
    close_rkisp_ctx();
    rkisp_ctx  = rkisp_open_device(device, 0);
    rkisp_set_fmt(rkisp_ctx, rkisp_ctx->width,rkisp_ctx->height , V4L2_PIX_FMT_YUV420);
    printf("----------------------------------create_rkisp_ctx:%s------------\n",device);
}

const struct rkisp_api_buf* get_rkisp_frame(int timeout_ms)
{
    const struct rkisp_api_buf* buf = NULL;
    if(rkisp_ctx){
        return rkisp_get_frame(rkisp_ctx,timeout_ms);
    }
    return buf;
}
...
{
    return rkisp_ctx ? rkisp_ctx->width : -1;
}

int get_rkisp_ctx_height()
{
    return rkisp_ctx ? rkisp_ctx->height : -1;
}
...
int camInit(const char *devicename)
{
    if (strcmp(devicename, "") == 0) {
        create_rkisp_ctx("/dev/video1");
//        set_video_name("/dev/video1");
    }
    else{
        create_rkisp_ctx(devicename);
//        set_video_name("/dev/video1");
    }
    int acodec_ind = encoder_get_audio_codec_ind_name("vorb");
...
//        fprintf(stderr, "cheese: invalid audio codec '%s' using pcm input\n", my_config->audio_codec);
    ...
    /*初始化音频context*/
    audio_context_t *audio_ctx = create_audio_context(audio, -1);
